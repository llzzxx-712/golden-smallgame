 ...
 */
export function buyItem(state, itemId, location = 'camp', price = null) {
  const template = ITEM_TEMPLATES[itemId];
  if (!template) return { success: false, message: '未知道具' };

  const finalPrice = price !== null ? price : (location === 'caravan' 
    ? Math.round(template.price * (0.6 + Math.random() * 0.8)) 
    : template.price);

  if (state.player.coins < finalPrice) {
  ...
  state.player.coins -= finalPrice;
  state.player.items.push({ id: itemId, ...template });
  return { success: true, message: `购买了 ${template.name}，花费 ${finalPrice} 💰` };
}
